...
public class IntegracaoWorker {
    ...
    public boolean processarPedidoEspecifico(Long idPedido) {
        try {
            var pedidoOpt = pedidoRepository.buscarPorId(idPedido);
            if (pedidoOpt.isEmpty()) {
                logger.error("Pedido não encontrado: {}", idPedido);
                return false;
            }
            return processarPedido(pedidoOpt.get());
        } catch (Exception e) {
            logger.error("Erro ao processar pedido específico {}: {}", idPedido, e.getMessage());
            return false;
        }
    }
